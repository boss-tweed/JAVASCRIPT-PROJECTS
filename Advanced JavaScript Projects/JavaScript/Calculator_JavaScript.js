...
function Input_Digit(digit){
    ...
    //to the key it was clicked on
    if (Wait_Second_Operand === true){
        Calculator.Display_Value = digit;
        Calculator.Wait_Second_Operand = false;
    } else {
        //This overwrites Display_Value if the correct value is 0
...
function Handle_Operator(Next_Operator){
    ...
    //When an operator key is prssed we convert the current number
    ...
    if (First_Operand == null){
        ...
        //If operaor exists, property lookup is performed for the operator
...
keys.addEventListener("click", (event) => {
    ...
    //that was clicked
    const { target } = event;
    //If element that is clicked on is not a button, exit the function
